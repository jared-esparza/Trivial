...
if (filter_var($email, FILTER_VALIDATE_EMAIL) === false || strlen($password) < 10) {
    fwrite(STDERR, "Uso: php bin/create-admin.php --email=admin@example.com --password=una-clave-de-10-caracteres\n");
    exit(1);
}
...
if ($user === null) {
    $user = $users->create($email, password_hash($password, PASSWORD_DEFAULT), 'admin');
} else {
