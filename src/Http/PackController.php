...
final class PackController
{
    ...
    public function registerRoutes(ApiRouter $router): void
    {
        ...
        $router->add('GET', '/packs/colors', fn (ApiRequest $request): ApiResponse => new ApiResponse(['colorSchemes' => $this->packs->listColorSchemes()]));
    ...
    {
        $this->requireAdmin($request);
        $this->packs->softDeleteColorScheme((int) ($request->body['colorSchemeId'] ?? 0));
        return new ApiResponse(['ok' => true]);
    }
